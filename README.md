...
- **Stealth fetching**: `HtmlScraper` handles plain server-rendered HTML and JSON APIs (the BOP path uses a structured, custody-aware JSON verifier rather than text matching); `WebViewScraper` drives a hidden Android-UA WebView to render JS-heavy pages. Both run only against automatable (non-`MANUAL_LANDING`) sources.
